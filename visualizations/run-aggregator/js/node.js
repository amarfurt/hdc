/**
 * Get records to be displayed with the run aggregator visualization.
...
var process = function(request, response) {
	...
	if (request.method === "GET" && parts[1] === "run-aggregator") {
			...
			parseRequestBody(getResponse, function(json) {
				if (!json.error) {
					...
					for (var i = 0; i < json.length; i++) {
						var recordString = json[i].data;
						if (recordString) {
							try {
								var data = JSON.parse(recordString).data;
							} catch (exception) {
								// skip
								continue;
							}
					...
					var result = {"distance": distance, "time": time, "speed": speed};
					ok(response, origin, JSON.stringify(result));
				} else {
