            ...
            $.ajax({
                    ...
                    success: function(response) {
                        $scope.$apply(function() {
                            ...
                            $scope.data[rs].userHas = $scope.records[$scope.selectedRecordName].snpMap.hasOwnProperty(rs);

                            if ($scope.data[rs].userHas) {
                                if ($scope.data[rs].snpediaOrientation === "minus") {
                                    $scope.data[rs].orientation = "minus";
                                    $scope.data[rs].genotype = $scope.changeOrientation($scope.records[$scope.selectedRecordName].snpMap[rs]);
                                } else {
                                    $scope.data[rs].orientation = "plus";
                                    $scope.data[rs].genotype = $scope.records[$scope.selectedRecordName].snpMap[rs];
                                }
                            }
                        });
                    }
